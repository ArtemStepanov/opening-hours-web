...
class OpeningHoursServiceImpl : OpeningHoursService {
    ...
    private fun test(request: ConvertOpeningHoursRequest): List<String> {
        LOGGER.trace("Validating request parameters")

        val errors = mutableListOf<String>()
        if (!validateRequestParameters(request.data, errors)) {
            LOGGER.trace("Some of request parameters are invalid")
            return errors
        }

        val sortedFlat = request.data.flatMap { (day, list) ->
            list.map { l ->
                Flat(
                    dayOfWeek = day,
                    type = l.type,
            ...
            .sortedWith(compareBy({ it.dayOfWeek }, { it.value }))
            .toMutableList()

        var previousFlat: Flat? = null
        for (currentFlat in sortedFlat) {
            ...
            when {
                previousFlat.type == currentFlat.type -> {
                    errors.add(
                        getOpeningErrorMessage(
                            currentFlat.dayOfWeek,
                            currentFlat.value,
                            if (currentFlat.type == OpeningType.OPEN) {
                                OpeningType.CLOSE
                            } else {
                                OpeningType.OPEN
                            }
        ...
        val first = sortedFlat.first()
        val last = sortedFlat.last()
        if (first.type == last.type) {
            errors.add(
                "No ${
                    (if (first.type == OpeningType.CLOSE) "opening entry for the day '${
                        DateTimeUtils.getDayOfWeekDisplayName(first.dayOfWeek)
                    }' and hour '${DateTimeUtils.fromUnixTimeToString(first.value.toLong())}'" else "closing entry for the day '${
                        DateTimeUtils.getDayOfWeekDisplayName(last.dayOfWeek)
                    }' and hour '${DateTimeUtils.fromUnixTimeToString(last.value.toLong())}'")
                }"
            )
        }

        if (first.type == OpeningType.CLOSE) {
            sortedFlat.add(sortedFlat.removeFirst())
        }
